...
import { fetchPublishers } from "../../services/serviceApi";
...
const Publishers = () => {
  ...
  const getDevelopers = async () => {
    try {
      setIsLoading(true);
      const fetchedData = await fetchPublishers(currentPage);
  ...
  const handleLoadMore = async () => {
    try {
      setIsLoadMore(true);
      const fetchedData = await fetchPublishers(currentPage + 1);
